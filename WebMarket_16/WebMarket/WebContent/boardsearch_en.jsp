...
<script type="text/javascript">
	...
	function boardsearch() {
		var form = document.member;
		var url = 'boardsearch_en.jsp?search=' + encodeURIComponent(form.search.value);
		location.href = url;
...
<body>
	...
	<div class="container">
		...
		<br>
		<form action="board_en.jsp" method="post" name="member">
			<table class="table table-hover">
				...
				<tbody>
					...
					%>
					<tr>
						<td><a href="./boardshow_en.jsp?title=<%=board.getNumber()%>&page=<%=currentPage %>&search=<%=search%>"><%=(currentPage - 1) * 10 + num%></a></td>
						<td><a href="./boardshow_en.jsp?title=<%=board.getNumber()%>&page=<%=currentPage %>&search=<%=search%>"><%=board.getTitle()%></a></td>
			...
			<div style="text-align: center;">
				<%
				if (currentPage > 1) {
					out.print("<a href='boardsearch_en.jsp?page=" + (currentPage - 1) + "&search="+search+"'>&lt; previous</a>");
				}
				for (int i = 1; i <= totalPageCount; i++) {
					if (i == currentPage) {
						out.print("<b><a class='active' href='boardsearch_en.jsp?page=" + i + "&search="+search+"'>" + i + "&nbsp</a></b>");
					} else {
						out.print("<a href='boardsearch_en.jsp?page=" + i + "&search="+search+"'>" + i + "&nbsp</a>");
					}
				}
				if (currentPage < totalPageCount) {
					out.print("<a href='boardsearch_en.jsp?page=" + (currentPage + 1) + "&search="+search+"'>next &gt;</a>");
				}
				%>
			</div>
