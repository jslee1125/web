...
<script type="text/javascript">
	function goToBoardChange(title, number) {
		var url = 'announcementchange_en.jsp?title=' + encodeURIComponent(title)
...
<body>
	...
	<div class="container">
		...
		<%if(search == null){ %>
		<form action="announcement_en.jsp?page=<%=currentPage %>" method="post">
		<%}else{ %>
		<form action="announcementsearch_en.jsp?page=<%=currentPage %>&search=<%=search %>" method="post">
		<%} %>
			<%
			if (writer.equals(id)) {
			%>
