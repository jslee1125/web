		...
		<form action="boardevent.jsp" method="post" name="member">
			<table class="table table-hover">
				...
				<tbody>
					...
					<tr>
						<td><a
							href="./boardeventshow.jsp?title=<%=board.getNumber()%>&page=<%=currentPage %>&search=<%=search %>"><%=(currentPage - 1) * 10 + num%></a></td>
						<td><a
							href="./boardeventshow.jsp?title=<%=board.getNumber()%>&page=<%=currentPage %>&search=<%=search %>"><%=board.getTitle()%></a></td>
			...
			<div style="text-align: center;">
				<%
				if (currentPage > 1) {
					out.print("<a href='boardeventsearch.jsp?page=" + (currentPage - 1) + "&search="+search+"'>&lt; 이전</a>");
				}
				for (int i = 1; i <= totalPageCount; i++) {
					if (i == currentPage) {
						out.print("<b><a class='active' href='boardeventsearch.jsp?page=" + i + "&search="+search+"'>" + i + "&nbsp</a></b>");
					} else {
						out.print("<a href='boardeventsearch.jsp?page=" + i + "&search="+search+"'>" + i + "&nbsp</a>");
					}
				}
				if (currentPage < totalPageCount) {
					out.print("<a href='boardeventsearch.jsp?page=" + (currentPage + 1) + "&search="+search+"'>다음 &gt;</a>");
				}
				%>
			</div>
